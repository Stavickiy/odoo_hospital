...
class MoveDoctorVisitWizard(models.TransientModel):
    ...
    available_slot_id = fields.Many2one('hospital_management.doctor.schedule',
                                        ...
                                        required=True)
    # new_start_datetime = fields.Datetime(string='New Visit start Date and Time',
    #                                      required=True)
    # new_stop_datetime = fields.Datetime(string='New Visit stop Date and Time',
    #                                     required=True)
    ...
    def move_doctor_visit(self):
        ...
        # Освобождаем старый слот (меняем его статус на available)
        ...
        # Обновляем запись на приём на новые данные
        visit.write({
        ...
        # Бронируем новый слот (меняем его статус на booked)
